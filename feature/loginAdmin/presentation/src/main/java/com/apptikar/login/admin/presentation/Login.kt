...
import com.apptikar.login.admin.presentation.R
...
fun Login(
   ...
   val password = loginViewModel.password.collectAsState()
    ...
    val status = rememberSaveable { mutableStateOf(connectivityStatus)}
        ...
        LaunchedEffect(key1 = userInfo.value){
            ...
            if (userInfo.value?.status == 1 ) {
                ...
                = NavDeepLinkRequest.Builder
                  .fromUri(navigateTo.toUri()).build()
                navController.navigate(deepLinkRequest)


            } else {
                if (userInfo.value?.message?.isNotEmpty() == true){
                    Toast.makeText(
                        context,
        ...
        TextButton(
            onClick = {
                val deepLinkRequest
                    ...
                    .fromUri(navigateTo.toUri()).build()
                navController.navigate(deepLinkRequest)
//                if (status.value == ConnectivityObserver.Status.Lost){
